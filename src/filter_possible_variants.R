...
option_list <- list(
  make_option(c("-i", "--input"),
    type = "character", default = NULL, help = "Input file path"),
  make_option(c("-o", "--output"),
    type = "character", default = NULL, help = "Output file path")
)
...
if (is.null(opt$input) || is.null(opt$output)) {
  stop("Please provide input and output file paths using -i/--input and -o/--output options.") # nolint
}
...
forward_strand_cpg <- dt[
  ref == "C" & shift(ref, type = "lead", fill = "") == "G"]

# Filter rows where REF is a "G" and the previous base is a "C"
reverse_strand_cpg <- dt[
  ref == "G" & shift(ref, type = "lag", fill = "") == "C"]
...
# Write output data
fwrite(cpg_dt, opt$output, sep = "\t", row.names = FALSE)
