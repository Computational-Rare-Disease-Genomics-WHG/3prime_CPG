...
from pyfaidx import Fasta  # pylint: disable=import-error


def get_context(reference, chrom, pos, length):
    """
    Get context of variant
    """
    start = pos - (length // 2)
    end = pos + (length // 2)
...
def main(args):
    ...
    with open(input_file, 'r', encoding='utf8') as infile, open(
        output_file, 'w', encoding='utf8'
    ) as outfile:
        # Header for the output TSV
        header = ['chr', 'pos', 'variant_id'] + [
        ...
        for line in tqdm(infile.readlines()):
            ...
            contexts = [
                get_context(reference, chrom, pos, length) for length in context_lengths
            ]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Gets the context of each variant in the input TSV file.'
    )
    parser.add_argument('input_file', type=str, help='Input TSV file with variants.')
    parser.add_argument(
        'reference_fasta', type=str, help='Reference genome FASTA file.'
    )
    parser.add_argument('assembly_report', type=str, help='Assembly report file.')
    parser.add_argument(
        'output_file', type=str, help='Output TSV file with variant contexts.'
    )
